...
class JobController extends Controller
{
    ...
    public function index(){
         ...
         if($companies != 0 ){
            $companies = Company::get()->random($companies);
         }
...
//front search
    ...
    {
        $find = Job::find($request->id)->delete();
        return redirect()->back()->with('message','SucessFully Delete!');

    }


    public function disable_job($id)
    {
        $find = job::where('id',$id)->first();  
        $find->status = 0 ;
        $find->save();
        return redirect()->back()->with('message','SucessFully Disable Job!');
    
    }


    
    public function enable_job($id)
    {
        $find = job::where('id',$id)->first();  
        $find->status = 1;
        $find->save();
        return redirect()->back()->with('message','SucessFully Enable Job!');
    
    }


    public function accept($userid,$application_id)
    {

        $find = jobuser::where("user_id",$userid)->where('job_id',$application_id)->first();
        $find->status = 1;
        $find->save();
        return redirect()->back()->with('message','You selected this Candidate for an interview!');
        dd($find);
    }
